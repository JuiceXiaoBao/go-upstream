...
import (
	...
	log "github.com/juicesix/logging"
)
...
// watchKV 监视 KV 存储中的键的更改。预期的用例是向路由表中添加额外的路由命令。
func watchKV(logger *log.Logger, client *api.Client, path string, config chan string) {
...
// watchPrefix 监视 KV 存储中的前缀的更改。预期的用例是向路由表中添加额外的路由命令。
func watchPrefix(logger *log.Logger, client *api.Client, prefix string, config chan map[string]string) {
	var lastIndex uint64
